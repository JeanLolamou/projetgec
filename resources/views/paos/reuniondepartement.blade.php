...
@section('content')
 ...
 <div class="sh-pagetitle-title">
            ...
            <h2>Compte Rendu Département</h2>
        ...
        <div class="col-lg-12  ">
          ...
          <ol class="breadcrumb">
            <div class="col-sm-6 col-md-2 ">
                ...
                <a style="text-decoration:none" href="Liste-activités"><button  class="btn btn-outline-primary btn-block">

            <span class="title"></span><i class="fa fa-home"></i> Accueil</button></a>
                
              </div><!-- col-sm -->

               @if((Auth::user()->user_role==1)||(Auth::user()->user_role==6)||(Auth::user()->user_role==5)||(Auth::user()->user_role==7))
...
@endif
    ...
    <div class="modal-dialog">
      <div class="modal-content">
        ...
        <form method="POST" action="ajoutreuniondepartement" enctype="multipart/form-data">
                        @csrf
          ...
          <div class="panel panel-default">
            ...
            <div class="panel-body">
             <div class="table-responsive mg-t-25">
                ...
                <tr>
                  <td>{{$reuniondepartements->libelle}}</td>
                  <td>{{(new DateTime($reuniondepartements->date))->format("d/m/Y")}}</td>
                  <td>{{$reuniondepartements->debut_seance}}</td>
                  <td>{{$reuniondepartements->leve_seance}}</td>
                  <td>
                    <a target="_blank" class="btn btn-success" href="{{route('pdfreuniondepartement',[$reuniondepartements->id])}}" title="Details" data-rel="tooltip">
                      <i class="fa fa-search-plus "></i>                                            
                    </a>
                   @if((Auth::user()->user_role==1)||(Auth::user()->user_role==6)||(Auth::user()->user_role==5)||(Auth::user()->user_role==7))
                     <a class="btn btn-warning" href="{{ route ('parametrerReuniondepartement', $reuniondepartements->id)}}" title="Parametrer" data-rel="tooltip">
                      <i class="fa fa-plus"></i>                                            
                    </a>

                    <a class="btn btn-info" href="{{ route ('editReuniondepartement', $reuniondepartements->id)}}" title="Modifier" data-rel="tooltip">
                      <i class="fa fa-edit "></i>                                            
                    </a>
                    <a class="btn btn-danger" data-toggle="modal" data-target="#myModal{{$reuniondepartements->id}}" title="Supprimer" data-rel="tooltip">
                      <i class="fa fa-trash-o " style="color: white;"></i> 
                    </a>
                    @endif
                     <!-- Suppression -->

                    <div class="modal fade" id="myModal{{$reuniondepartements->id}}">
    <div class="modal-dialog">
      <div class="modal-content">
        <div class="modal-header">
        ...
        <div class="modal-footer">
          <form action="{{ route ('Reuniondepartement.update', $reuniondepartements->id)}}" method="post" >
               {{ csrf_field() }}
              {{ method_field('PUT') }}
          <button type="button" class="btn btn-default" data-dismiss="modal">Annuler</button>
