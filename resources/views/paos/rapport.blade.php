...
@section('content')
 ...
 @if((Auth::user()->user_role==5)||(Auth::user()->user_role==6)||(Auth::user()->user_role==3))
                <a style="text-decoration:none" href="addrapport"><button  class="btn btn-outline-primary btn-block">
...
<div class="card body">
          ...
          <label class=" control-label" for="date"><b>Date &nbsp;</b></label>
                      ...
                      @if(isset($exist_date))
                               <input type="date" id="input-small" name="date" class="form-control input-sm" value="{{$exist_date}}">
                              @else
                              <input type="date" id="input-small" name="date" class="form-control input-sm" placeholder="">
                              @endif
                            ...
                            <select id="mois" name="mois" class="form-control" size="">
                             ...
                             @if(isset($exist_mois) and ($exist_mois!=-1) and ($rapports->id==$exist_mois))
           <option value="{{$rapports->id}}" selected>{{$rapports->mois}}</option>
          @else
          <option value="{{$rapports->id}}">{{$rapports->mois}}</option>
          @endif
                            @endforeach
                                </select>
           ...
           <option value="{{$rapports->id}}" selected>{{$rapports->semaine}}</option>
          @else
          <option value="{{$rapports->id}}">{{$rapports->semaine}}</option>
          @endif
                            @endforeach
                                </select> 
                          ...
                          <button type="submit" class="btn btn-sm btn-success"><i class="fa fa-dot-circle-o"></i> Filtrer</button>
                                <br>
                    <a target="_blank" href="{{ route('export_rapport').'?'.$query }}" 
           ...
           <br>
                     ...
                     <a class="btn btn-danger" href="/globalrapport">Rapport Global</a>
                     @endif
                     @if(Auth::user()->user_role==5)
                     <a class="btn btn-danger" href="/manglobalrapport">Rapport Global</a>
                     @endif
              ...
              <div class="table-responsive mg-t-25">
                ...
                <thead class="bg-info">
                  <tr style="background: #0b0e69;color: white;">
                    ...
                    <th>Direction</th>
                    <th>Responsable</th>
                    <th>Titre du Rapport Hebdomadaire</th>
...
<tfoot class="bg-info">
                ...
                <tr>
                  ...
                  <!--  <td style="width:5px;">{!!$rapports->rapport!!}</td>  -->
                  ...
                  <td>
                    <a class="btn btn-success" href="{{ route ('rapportshow', $rapports->id)}}" title="Details" data-rel="tooltip">
                     ...
                     <a target="_blank" href="{{route('pdfrapportunique', $rapports->id)}}" class="btn btn-sm btn-warning" title="Imprimer" data-rel="tooltip"><i class="fa fa-print"></i>
                     </a>
@if((Auth::user()->user_role==5)||(Auth::user()->user_role==6)||(Auth::user()->user_role==3))
